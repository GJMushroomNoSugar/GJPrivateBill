//
//  GJPBDayBookTableViewController.swift
//  GJ Private Bill
//
...
class GJPBDayBookTableViewController: UITableViewController {

    var viewModel: GJPBDayBookViewModel
    ...
    init(time: String) {
        viewModel = GJPBDayBookViewModel(time: time)
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = viewModel.time
    ...
    @objc func reloadInfo() {
        ...
        //刷新数据
        viewModel.requestInfo(success: { [unowned self] in
    ...
    func reloadData() {
        ...
        totalMoneyLabel.text = viewModel.getTotalMoney()
        totalMoneyLabel.textColor = (Int(viewModel.getTotalMoney()) ?? 0) >= 0 ? UIColor(hex: "#225DFC"): UIColor.orange
    }
