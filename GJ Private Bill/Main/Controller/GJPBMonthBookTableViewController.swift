//
//  GJPBMonthBookTableViewController.swift
//  GJ Private Bill
//
...
class GJPBMonthBookTableViewController: UITableViewController {

    var viewModel: GJPBMonthBookViewModel
    
    //本月收入
    private lazy var allIncomeLabel: UILabel = {
        let label = UILabel()
        label.font = UIFont.systemFont(ofSize: 17)
        let str = "本月收入：0  本月支出：0"
        
        let attributeStr: NSMutableAttributedString = NSMutableAttributedString(string: str)
        attributeStr.addAttributes([NSAttributedString.Key.foregroundColor : UIColor(hex: "#225DFC")], range: NSRange(location: 5, length: 1))
        attributeStr.addAttributes([NSAttributedString.Key.foregroundColor : UIColor.orange], range: NSRange(location: 13, length: 1))
    ...
    init(year: String) {
        viewModel = GJPBMonthBookViewModel(year: year)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        navigationItem.title = viewModel.year
    ...
    @objc func reloadInfo() {
        ...
        //刷新数据
        viewModel.getMonthRecord(success: { [unowned self] in
    ...
    func reloadData() {
        ...
        let str = "本月收入：\(viewModel.model?.allIncome ?? "0")  本月支出：\(viewModel.model?.allSpending ?? "0")"

        let attributeStr: NSMutableAttributedString = NSMutableAttributedString(string: str)
        attributeStr.addAttributes([NSAttributedString.Key.foregroundColor : UIColor(hex: "#225DFC")], range: NSRange(location: 5, length: (viewModel.model?.allIncome ?? "0").count))
        attributeStr.addAttributes([NSAttributedString.Key.foregroundColor : UIColor.orange], range: NSRange(location: 12 + (viewModel.model?.allIncome ?? "0").count, length: (viewModel.model?.allSpending ?? "0").count))
...
//        navigationController?.pushViewController(BILLDayBookTableViewController(info: model!.list![indexPath.row]), animated: true)
    }
